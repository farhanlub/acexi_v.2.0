            ...
            <div class="col-md-8"> 
                <div class="card">
                    <div class="card-body animated-form">
                        <form action="{{ route('admin-tren-terbaru-update', $news->id) }}" method="POST" enctype="multipart/form-data" class="row g-3 needs-validation custom-input">
                            @csrf
                            @method('PUT') 
                            <div class="col-12">
                                ...
                                <select class="form-control" name="program_kegiatan_id" id="program_kegiatan_id" required>
                                    ...
                                    @foreach ($program as $p)
                                        <option value="{{ $p->id }}" {{ $news->tren_category_id == $p->id ? 'selected' : '' }}>
                                            {{ $p->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-12">
                                ...
                                <input class="form-control" name="content" id="content" type="url" required value="{{ old('content',$news->content) }}">
                            </div>
                            <div class="col-12">
                            ...
                            <div class="col-12">
                                @include('components.buttonForm', ['route' => 'admin-tren-terbaru'])
                            </div>
                        </form>
                    </div>
                </div>
