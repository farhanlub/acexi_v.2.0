            ...
            <div class="col-md-6">
                <div class="card">
                    <div class="card-body animated-form">
                        <form action="{{ route('admin-akun-pengurus-update', $pengurus->id) }}" method="POST" class="row g-3 needs-validation custom-input">
                            @csrf
                            @method('PUT')
                            <div class="col-12 position-relative">
                                ...
                                <select name="pengurus_id" class="select2 form-control" id="validationTooltip04" required="">
                                    <option selected value="{{ $pengurus->pengurus->id }}">{{ $pengurus->pengurus->name }}</option>
                                    @foreach ($allPengurus as $item)
                                        <option value="{{ $item->id }}" {{ $pengurus->pengurus_id == $item->id ? 'selected' : '' }}>{{ $item->name }}</option>
                                    @endforeach
                                </select>
                                @error('pengurus_id')
                            ...
                            <div class="col-12">
                                @include('components.buttonForm', ['route' => 'admin-akun-pengurus'])
                            </div>
                        </form>
                    </div>
                </div>
