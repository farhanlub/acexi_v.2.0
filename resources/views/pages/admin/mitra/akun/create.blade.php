...
@section('content')
    ...
    <div class="container-fluid">
                ...
                <div class="col-md-6">
                    <div class="card">
                        <div class="card-body animated-form">
                            <form action="{{ route('admin-akun-mitra-store') }}" method="POST" class="row g-3 needs-validation custom-input">
                                @csrf
                                @method('POST')
                                <div class="col-12 position-relative">
                                    ...
                                    <select name="pengurus_id" class="select2 form-control" id="validationTooltip04" required="">
                                        ...
                                        @foreach ($members as $item)
                                            <option value="{{ $item->id }}" {{ old('pengurus_id') == $item->id ? 'selected' : '' }}>{{ $item->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('pengurus_id')
        ...
        @else
            <div class="alert alert-danger">
                <p>Belum ada mitra yang terdaftar atau mitra yang memiliki alamat email. Silahkan tambahkan mitra atau alamat email terlebih dahulu.</p>
            </div>
            <a href="{{ route('admin-data-mitra') }}" class="btn btn-primary">Lihat data mitra</a>
        @endif 
    </div>
@endsection
